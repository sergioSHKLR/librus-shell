...
/** Last caption i18n key — re-spoken on unmute. */
let lastOnboardCaptionKey = "";
...
/**
 * Pick best available pt voice, or null to leave voice unset (OS default).
 * Prefers Google/Microsoft/cloud; never forces known-bad enginge names.
...
function pickPtVoice() {
  try {
    ...
    for (const v of voices) {
      ...
      if (/microsoft|neural|natural|wavenet|studio/i.test(name)) score += 80;
      if (/luciana|maria|francisca|daniela|heloisa|brasil|brazil/i.test(name))
        score += 20;
...
/**
 * Must run inside a click/tap handler on ChromeOS — later async speak() is
 * otherwise silent. Primes the engine and optionally speaks caption text now.
 * @param {string} [text]
 */
function unlockOnboardSpeech(text) {
  if (
  ...
  try {
    /* Cancel any stuck queue, then speak in this gesture turn. */
    ...
    if (voice && (voice.localService === false || /google|microsoft|neural/i.test(voice.name))) {
      u.voice = voice;
      u.lang = voice.lang || "pt-BR";
    }
    speechSynthesis.speak(u);
    if (!line) {
      /* Silent prime — drop it so it doesn't block the next caption. */
      setTimeout(() => {
        try {
          speechSynthesis.cancel();
        } catch (_) {
...
function toggleOnboardMute() {
  const turningOn = onboardSpeechMuted; /* currently muted → will unmute */
  setOnboardSpeechMuted(!onboardSpeechMuted);
  if (!turningOn || __onboardGetLang() !== "pt") return;
  const line =
    document.getElementById("onboard-caption")?.textContent?.trim() ||
    (lastOnboardCaptionKey ? t(lastOnboardCaptionKey) : "");
  /* Gesture-safe: speak (or prime) immediately on unmute click. */
  unlockOnboardSpeech(line || "");
  if (line) {
    /* Keep token/resume bookkeeping aligned with unlock speak. */
    onboardSpeechToken += 1;
 ...
 * Speaks ASAP (no multi-second voice wait) so ChromeOS does not drop audio
 * after the user-gesture window closes.
 * @param {string} text
...
function speakOnboardCaption(text) {
  ...
  return new Promise((resolve) => {
    ...
    const speakNow = () => {
      ...
      u.pitch = 1;
      /*
       * Only assign an explicit voice when it looks like a cloud/neural
       * engine. Otherwise leave unset so ChromeOS system TTS can choose.
       */
      if (
        voice &&
        (voice.localService === false ||
          /google|microsoft|neural|natural|wavenet|studio/i.test(voice.name || ""))
      ) {
        u.voice = voice;
        u.lang = voice.lang || "pt-BR";
      }
      ...
      u.onerror = (ev) => {
        try {
          console.warn("[onboard TTS] error", ev?.error, {
            voice: voice?.name,
      ...
      try {
        console.info(
          "[onboard TTS]",
          u.voice ? `voice=${u.voice.name}` : "lang=pt-BR OS default",
      ...
      try {
        speechSynthesis.speak(u);
        /* Some Chrome builds start paused — kick resume. */
        try {
        ...
        /* Safety: if engine never fires end/error, don't block the How-to loop */
        setTimeout(() => {
          if (token === onboardSpeechToken) finish();
        }, Math.min(60000, 2000 + trimmed.length * 80));
      } catch (_) {
        finish();
      }
    };

    /* Brief defer after cancel() — Chrome drops same-turn speak — but keep
     * it short so we stay near any preceding user gesture (play/unmute). */
...
function setHowPaused(paused) {
  ...
  if (!howPaused) {
    /* Resume click = user gesture — unlock TTS for ChromeOS. */
    if (!onboardSpeechMuted && __onboardGetLang() === "pt") {
      unlockOnboardSpeech("");
    }
...
function startHowRevealAnim() {
  ...
  syncOnboardMuteButton();
  /*
   * Play click is a user gesture — prime speechSynthesis here or ChromeOS
   * stays silent for later async caption speaks.
   */
  if (!onboardSpeechMuted && __onboardGetLang() === "pt") {
    unlockOnboardSpeech(t("onboard.cap.howStart"));
  } else {
    try {
      speechSynthesis.getVoices?.();
    } catch (_) {
