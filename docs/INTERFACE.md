# Interface freeze — v0.9.11

**Date:** 2026-09-10  
**Version:** `0.9.11` BETA  
**Repo:** `librus-shell` (build step 3 of 4)

Chrome, Help, flavors, and Consulte empty-state are **frozen** (v0.9.9). **v0.9.10** is handheld/toolbar polish on that freeze. **v0.9.11** stops Android Fullscreen API auto-enter. Do not add panes, bar items, Help beats, or flavor marks without an explicit unfreeze. Book HTML/CSS and editorial Markdown are **not** frozen.
...
5. **BETA** — Settings → Sobre, next to `v0.9.11`.  
...
9. **Viewport** — phone landscape blocked (rotate to portrait). Portrait phone: Leia + Ache overlay; Consulte off. Names: Consult not Context.  
   ...
   Do **not** auto-call the Fullscreen API on Android installed PWAs — Chrome + Motorola immersive mode flashes black and shows “drag from the top and touch the back button.” `#book-fs` is the only enter path; `navigationUI: hide` is skipped on Android. Manifest `orientation` is `any`. Status `theme-color` follows the Leia tab band in the reader so a leftover bar isn’t a grey strip.  
...
4. **Barra** — Biblioteca: só Ajuda · Ajustes. Leitor: Home › livro. BETA em Ajustes, ao lado de `v0.9.11`.  
...
7. **Tela cheia** — PWA standalone; não chamar a Fullscreen API sozinha no Android. Botão à direita em Páginas (não em Tipo).  
